...
def test_station_lifecycle():
    ...
    try:
        # Create station
        station_name = f"integration-test-station-{datetime.now().strftime('%Y%m%d%H%M%S')}"
        ...
        created_station = client.stations.create(str(created_campaign.id), station_create)
        assert created_station.id is not None
        print(f"Created station: {created_station.id}")

        try:
            # Get station
            fetched_station = client.stations.get(str(created_station.id), str(created_campaign.id))
            ...
            # Update station
            station_update = StationUpdate(description="Updated integration test station")
            client.stations.update(str(created_station.id), str(created_campaign.id), station_update)

            # Fetch again to verify update
            fetched_again = client.stations.get(str(created_station.id), str(created_campaign.id))
            assert fetched_again.description == "Updated integration test station"
            print(f"Updated station: {fetched_again.id}")

        finally:
            # Delete station
            deleted = client.stations.delete(str(created_station.id), str(created_campaign.id))
